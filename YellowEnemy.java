...
public class YellowEnemy implements Enemy {

	public static final String img_file = "YellowBadGuy.png";
	...
	public YellowEnemy(int x, int y, int playfield_h, int playfield_w, int origY, int origX) {
		this.scoreStationary = 50;
		this.scoreFlying = 100;
		this.flyingState = 1;
		this.xCoordinate = x;
		this.yCoordinate = y;
	...
	/** 
	 * Returns the current yCoordinate of this particular yellow enemy
	 */
	@Override
	public int getY() {
	...
	/** 
	 * Returns the current xCoordinate of this particular yellow enemy
	 */
	@Override
	public int getX() {
		// TODO Auto-generated method stub
	...
	public TheirMissiles fire() {
		...
		if (this.missileScreenCount != 0) {
			...
			TheirMissiles missile = new TheirMissiles(this.xCoordinate, this.yCoordinate, 
					null, null, this, this.getPlayfieldH, this.getPlayfieldW);
			return missile;
		}
		return null;
	}
	...
	/**
	 * So right now, for sake of simplicity, the collision detection will be bounding
	 * box based, much like in Mushroom Of Doom. However, once the rest of the game works, this
	 * will be changed so that it's truly sprite based (where if any non-black part of the pixel
	 * will cause the whole sprite to be modified if there is a collision with certain
	 * objects).
	 * 
	 */
	...
	/**
	 * Much like in the original 1981 game, enemy-enemy collisions are simply not
	 * a thing. For my observations, there is code to simply avoid collisions,
	 * and / or if there is one, nothing really happens. The only collision that matters
	 * are the ones above.
	 */
